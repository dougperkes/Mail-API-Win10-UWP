...
4. Now let's add an inbox header and refresh button as another grid inside of the first grid and a progress indicator to display while the messages are being retrieved. Add the following just after the `</Grid.ColumnDefinitions>` tag:
...
5. Now, let's add a list to display the inbox. We'll use a ListView along with the template we created earlier. Add the following after the `<ProgressRing />` element:
...
6. Now, let's wire up some code to retrieve emails. In Solution Explorer, Right-click MainPage.xaml and select View Code. Add a new method to load emails:
...
7. Now, let's create a method for the refresh button click event, call the load method when the page loads and a blank click handler for the list:
...
Congratulations! You just built a working Office 365 inbox viewer in Windows 10.
